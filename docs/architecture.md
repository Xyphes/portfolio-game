...
Phaser continues to own sprites, movement, collisions, and spatial interactions, but no longer rasterizes interface text into the scaled canvas. A pointer-transparent React layer renders the localized zone title, object labels, action hint, tutorial state, and awaiting-content message from the same validated content and layout coordinates. The concise action hint explicitly identifies interaction and lists mouse, touch, and keyboard inputs in both languages; its panel shrink-wraps the localized copy instead of reserving a fixed portion of the game width. Container-relative typography tracks every supported game-size tier while remaining browser-rendered and sharp; no editorial text or progress data is duplicated inside Phaser.
...
The deterministic field contains 40 green squares and individually dispersed golden triangles on wide screens and its density is halved to 20 on narrow screens; the existing global reduced-motion policy collapses the animation to a static composition. The triangles use a warm three-stop gradient, a two-level glow, a minimum 14-pixel footprint, and a dedicated higher-opacity motion cycle so their silhouette and gold color remain evident against the dark background, but they never form or reproduce a third-party game symbol. Particle movement uses continuously interpolated, GPU-composited transforms over roughly 2.9 to 4.4 seconds; the stepped pixel-art timing was rejected because it looked like dropped frames at this subtle scale.

## Jalon 31 sliding virtual directional pad

The four accessible directional buttons remain keyboard-operable, but their pointer interaction behaves as a single eight-way virtual joystick. The first pointer is captured by its initial button; movement is continuously resolved relative to the centre of the full pad, with a 12% central dead zone and 22.5-degree sector boundaries. Sliding from right toward up therefore changes direction without lifting, while diagonal sectors hold one horizontal and one vertical bridge direction simultaneously. Phaser already normalizes combined-axis velocity, so diagonal movement does not run faster.

Pointer release, cancellation, lost capture, input-disable state, and React unmount all clear held touch directions. The currently resolved arrow or arrow pair receives visual feedback, while keyboard, mouse destination movement, action input, and the typed React/Phaser bridge remain unchanged.

## Jalon 32 profile portrait

The user-provided portrait is a validated part of the canonical profile entity, with one stable public path and bilingual alternative text. Classic mode renders it only in the About category: beside the introduction on wide screens and directly below the title, before the introduction, on narrow screens. The original JPEG is copied unchanged and remains a small 76 KiB asset; CSS owns its responsive square crop focused toward the subject, subtle color treatment, green-and-gold frame, and decorative pixels.

The photo is also reused on the landing page: a framed portrait occupies the free upper-right desktop space without overlapping the language switch, while a compact centred version on narrow screens uses the vertical space recovered by removing forced equal-height mode cards. The hero's top padding is reduced so the name moves upward on desktop; mobile keeps its dedicated title width beside the right-aligned language switch. On wide layouts, the choice grid uses a bounded responsive height instead of consuming the entire remaining viewport, and the professional title gains a desktop-only separation below the large name. The portrait is suppressed in the intermediate tablet range where it could collide with the identity block, and remains absent from adventure mode so it cannot compete with the pixel-art world. The two mobile choice cards size to their content and retain their minimum 44-pixel targets. The photo's provenance and publication authorization are recorded in the asset register.

## Jalon 33 personal-interest galleries

The former generic crafting interest is split into the user-approved leathercraft, jewelry, and woodworking entities, and lock picking joins climbing, swimming, and travel. Every interest retains one stable canonical ID, a bilingual label, and an explicit photo-folder key. The personal adventure screen references all seven entities and uses a tested four-plus-three spatial layout that remains clear of the central obstacle and border foliage.

Classic mode presents the seven interests through a compact accessible activity selector and mounts only the selected gallery. This avoids decoding every category at once and replaces the previous unequal stack of large cards. The selected activity uses a bounded single-media carousel, so only the current photo or video exists in the DOM and the desktop section fits within the application viewport instead of requiring a long gallery scroll. Previous and next controls wrap around the mixed media set; numbered indicators support direct selection, arrow keys work from the focused carousel without stealing native video controls, and a horizontal pointer gesture changes photos on touch devices. Mobile keeps its own scrollable activity rail and bounded stage, while empty categories show one concise localized message. Videos have native controls, inline playback, `preload="none"`, and no autoplay; they are not included in the PWA precache.

Original user media remain in `src/assets/interests`. The explicit `npm run optimize:photos` pipeline applies EXIF orientation, limits JPEG and PNG display copies to 1600 pixels on their longest edge, and writes them to the parallel `src/assets/interests-optimized` tree. Vite discovers the optimized photos and original MP4/WebM videos at build time; alphabetical filenames provide deterministic mixed-media ordering, and inactive categories create no media elements. The current 17-photo set falls from 35.23 MB of originals to 2.51 MB of display assets. Videos are not transcoded by the photo pipeline: the current 36.56 MB climbing MP4 and 8.13 MB jewelry MP4 therefore require a deliberate play action before loading, and should be separately compressed for quicker playback. Gallery photos currently use generic bilingual alternative text, and videos generic bilingual accessible names, because no individual captions or descriptions have been supplied; their publication rights and provenance remain recorded in the asset register.

On narrow screens, the classic-mode burger and overlay menu are replaced by a persistent horizontally scrollable section ribbon directly below the identity/language header. The active section remains visible when navigation changes, and the adventure escape link stays available at the ribbon's end. Desktop retains the existing sidebar navigation.
